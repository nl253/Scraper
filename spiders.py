...
from tests.test_spiders import BaseSpiderVerifier
from utils.logutils import *
from http_tools import validate_HTML, validate_URL
...
class BaseSpider(metaclass=ABCMeta):
    ...
    @staticmethod
    def _kill_children(children: Iterable[Process]):
    ...
    def add_URL(self, URL: str):
        """
        YOU DO NOT call this directly.
        All it does is simulate collections.Counter behaviour when adding a URL
        to List of URLs to be scraped.
		...
		key being the URL.
		It is very inefficient but ensures that relevant websites are visited.
		"""
    ...
    def _thread_start(self) -> int:
        """
        Called by each child process on each of it's threads.

        This is what generates the actual HTML documents and links and adds them to lists and queues.

         This is the core of each Spider.

         Looping takes place while enough entries have not been generated.
         Note that these threads will be killed by the main thread anyway if the timeout has passed.

         Also, if they repeatedly try to get something off an empty queue,
         they will be killed (they break and signal to the main thread to break the barrier which
         kills all threads in a child-process)
		"""
        ...
        def report():
            """
            A helper function to give info about the state of 'scraping'
            after each iteration within the while loop below.
            """
            proc_thread_warn(
        ...
        # while there is still data to generate
        while self._yielded_counter.value + self._results.qsize() <= self._max_results:
            ...
            try:
                focus_url: str = self.next_URL
                threadvars.misses: int = 0  # reset

                proc_thread_warn(
                    f'Focus URL: {focus_url} taken off URLs to scrape, {len(self.URLs)} items left')
            ...
            if not self._results.full():
                if self.test_result(wrapper.URL, wrapper.HTML, wrapper.headers):
                    ...
                    for URL in wrapper.iURLs:
                        self.add_URL(URL)
            else:  # if results are full (max specified in constructor)
